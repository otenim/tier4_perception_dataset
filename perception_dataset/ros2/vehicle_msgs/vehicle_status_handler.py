...
if TYPE_CHECKING:
    # Autoware overlay message types, used for type hints only. They live in real packages
    # (autoware_vehicle_msgs / tier4_vehicle_msgs) that are only importable when the ROS workspace
    # overlay is sourced, so importing them at runtime is avoided (mirroring ego_state_fields). The
    # actual messages are deserialized dynamically by Rosbag2Reader from the bag's type strings, so
    # these classes are never needed at runtime.
